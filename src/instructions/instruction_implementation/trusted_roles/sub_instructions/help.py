...
class Help(InstructionParent):
    ...
    def run(self, message):
        ...
        ret += "\n"
        ret += "`" + self.commandline_config.first_keyword + " auto-translation get` displays the roles that are allowed " \
               "to change the auto translation settings ( it does not ping them )\n"
        ret += "\n"
        ret += "`" + self.commandline_config.first_keyword + " auto-translation remove [roles]` removes given roles from " \
               "the allowed to change the auto translation\n"
        ret += "Usage is the same as the `set` instruction\n"
        ret += "Please note that the server owner will still be able to change the settings regardless of the roles he " \
               "has or has not\n"
        return ret
