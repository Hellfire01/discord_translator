...
class TranslateHelp(InstructionParent):
    ...
    def run(self, message):
        ...
        ret += "To use it, use the flag emotes, names or acronyms. The available flag emotes can be list with the `" +\
               GlobalSettings.get_instance().instruction_keyword[0] + " lang-list` instruction\n"
        ret += "The first flag is for the input languages, the following ones are the output languages\n"
        ret += "\n"
        ret += "Usage example :\n"
        ret += "**" + GlobalSettings.get_instance().instruction_keyword[0] + " translate :flag_fr: " + GlobalSettings.get_instance().translate_splitter + " :flag_us: :flag_de:**\n"
        ret += "This will attempt to take in french and translate it automatically to english and german\n"
        ret += "\n"
        ret += "Usage example :\n"
        ret += "**" + GlobalSettings.get_instance().instruction_keyword[0] + " translate en " + GlobalSettings.get_instance().translate_splitter + " french**\n"
        ret += "This will attempt to take in english and translate it automatically to french\n"
        ret += "\n"
        ret += "Usage example :\n"
        ret += "**" + GlobalSettings.get_instance().instruction_keyword[0] + " translate " + GlobalSettings.get_instance().translate_splitter + " it**\n"
